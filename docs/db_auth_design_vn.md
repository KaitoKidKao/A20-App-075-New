...
1. **Đăng ký**: Người dùng gửi thông tin tới API. Mật khẩu sẽ được hash (mã hóa một chiều) trước khi lưu vào DB.
2. **Đăng nhập**: Backend kiểm tra email và mật khẩu. Nếu đúng, trả về một chuỗi `access_token`.
3. **Truy cập API**: Với các API cần bảo mật (như upload video), người dùng phải gửi kèm `access_token` trong Header của request.

### 3.2. Công nghệ sử dụng
- **SQLModel**: Để tương tác với database theo hướng hướng đối tượng (ORM).
- **python-jose**: Để tạo và xác thực JWT Token.
- **passlib + bcrypt**: Để mã hóa mật khẩu an toàn.
